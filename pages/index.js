...
export default function IndexPage() {
  ...
  function submit(e) {
    ...
    if (email.match(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g)) {
      axios({
        method: 'get',
        url: `https://hook.integromat.com/9sa5ns4g7pycyrsk16dc6n68ibtts702?email=${email}`
      }).then(res => {
        const { data } = res

        const { myTickets = [], collabTickets = [] } = data
        setIsLoading(false)
  ...
  const { myTickets = [], collabTickets = [] } = user || {}

  return (
    <div>
      <Head>
        <title>My Marketing Requests</title>
        <meta name="description" content="Omnitas portal" />
      ...
      <main>
        {user ? (
          <>
            <SmallButton className="white" onClick={() => setUser(null)}>Log out<RiArrowRightSLine size="24" /></SmallButton>
            <MyTickets myTickets={myTickets} collabTickets={collabTickets} />
          </>
        ) : isLoading ? (
          <div className="loading">
            <h1 className="no-margin">Hold on while we retrieve your requests</h1>
            <ClipLoader />
            <span>This can take a few seconds</span>
          </div>
        ) : (
          <form className="login">
            <h1>Log in to view your marketing requests</h1>
            {error === 'invalid' && <span className="error">You need to submit an email</span>}
            {error === 'unauthorized' && <span className="error">Email does not exist</span>}
            <label htmlFor="frmEmailA" className="label">Email</label>
            <input
              type="email"
              name="email"
              id="frmEmailA"
              placeholder="name@nordicsemi.no"
              required
              autoComplete="email"
              value={email}
